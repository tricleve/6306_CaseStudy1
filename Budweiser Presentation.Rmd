...
colnames(brewery_count_by_state)[2] = "breweries_count"

#Merge/select brewery count with statepop (usmap data set) to get fips for plot.
plotbrew <- merge(statepop,brewery_count_by_state,by.x = "abbr", by.y = "State") %>% 
  select(fips, breweries_count)

#Plot brewery count on the state map.  Includes Alaska, Hawaii and DC
## How to display East States?
plot_usmap(data=plotbrew, values="breweries_count", color = "black", labels = TRUE) + 
  scale_fill_gradient(name = " Brewery Count", low = "yellow", high = "red") + 
  theme(legend.position = "right")

#Plot brewery count bars - descreasing count order

brewery_count_by_state_df <- as.data.frame(brewery_count_by_state)

plot(
  ggplot(data=brewery_count_by_state_df, aes(x=reorder(State, -breweries_count), y=breweries_count, fill= State)) +
    geom_col() +
    theme_bw() +
    xlab("State") +
